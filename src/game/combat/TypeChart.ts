...
// Gen 6+ type effectiveness chart. Only non-1x values stored; default is 1.
const CHART: Partial<Record<PokemonType, Partial<Record<PokemonType, number>>>> = {
  ...
  fighting: { poison: 0.5, flying: 0.5, psychic: 0.5, bug: 0.5, fairy: 0.5, ghost: 0.1, normal: 2, ice: 2, rock: 2, dark: 2, steel: 2 },
  poison:   { poison: 0.5, ground: 0.5, rock: 0.5, ghost: 0.5, steel: 0.1, grass: 2, fairy: 2 },
  ...
  ghost:    { normal: 0.1, dark: 0.5, ghost: 2, psychic: 2 },
  dragon:   { steel: 0.5, fairy: 0.1, dragon: 2 },
...
export class TypeChart {
  static getEffectiveness(attackType: PokemonType, defenderTypes: PokemonType[]): number {
    return defenderTypes.reduce((multiplier, defType) => {
      return multiplier * (CHART[attackType]?.[defType] ?? 1)
    }, 1)
  }
}
